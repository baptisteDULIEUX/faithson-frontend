...
// chemin de l'image dans public/images/boutique/
...
function handleClick() {
  if (isQuoteOnly.value) {
    // produit "sur devis" -> on renverra vers la section devis
  ...
  <article class="card">
    <div class="thumb">
      ...
      <div v-else class="ph-shape">
        <GarmentIcon
          :shape="product.placeholder?.shape"
          :color="product.placeholder?.color"
        />
      </div>

      <button class="qv" @click="handleClick">
...
.ribbon {
  ...
  padding: 0.35em 0.7em; border-radius: 3px;
}
.qv {
  position: absolute; bottom: 12px; left: 12px; right: 12px; z-index: 2;
  background: var(--ink); color: var(--cream); border: none; border-radius: 7px;
  padding: 0.7em; font-family: 'Karla'; font-weight: 700; font-size: 0.85rem;
  cursor: pointer; opacity: 0; transform: translateY(8px); transition: 0.18s;
}
